//! Integration tests for the dstate-ractor adapter.
//!
//! Covers ADAPT-01 through ADAPT-06 (adapter conformance) and RACTOR-02
//! through RACTOR-07 (ractor-specific implementation tests).

use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::{Arc, Mutex};
use std::time::Duration;

use dstate::{ActorRef, ActorRuntime, ClusterEvent, ClusterEvents, NodeId, TimerHandle};
...
// ---------------------------------------------------------------------------
// RACTOR-02: send() maps to ractor cast() — fire-and-forget
// ---------------------------------------------------------------------------

#[tokio::test]
async fn ractor_02_send_is_fire_and_forget() {
    let rt = RactorRuntime::new();
    let received = Arc::new(Mutex::new(Vec::new()));
    let r = received.clone();

    let actor = rt.spawn("ractor02", move |msg: String| {
...
// ---------------------------------------------------------------------------
// RACTOR-04: get_group_members returns typed refs
// ---------------------------------------------------------------------------

#[tokio::test]
async fn ractor_04_get_group_members() {
    let rt = RactorRuntime::new();
...
// ---------------------------------------------------------------------------
// RACTOR-06: Timer uses tokio under the hood
// ---------------------------------------------------------------------------

#[tokio::test]
async fn ractor_06_send_after_cancel_before_fire() {
    let rt = RactorRuntime::new();
    let count = Arc::new(AtomicU64::new(0));
    let c = count.clone();
...
// ---------------------------------------------------------------------------
// RACTOR-07: Re-exports core crate types
// ---------------------------------------------------------------------------

#[test]
fn ractor_07_reexports_core_types() {
    // Verify that the dstate crate is re-exported and types are accessible
    let _ = dstate_ractor::dstate::NodeId(1);
    let _config = dstate_ractor::dstate::StateConfig::default();

    // Verify adapter types are exported
    let _rt = dstate_ractor::RactorRuntime::new();
    let _events = dstate_ractor::RactorClusterEvents::new();
}

// ---------------------------------------------------------------------------
// Additional: RactorRuntime is Clone and Default
// ---------------------------------------------------------------------------
