//! Node Resource Monitor — ractor adapter example.
//!
//! Demonstrates how to define a `DeltaDistributedState` for tracking
//! per-node resource metrics (CPU, memory) and use `RactorRuntime` to
//! run actors that monitor and query those metrics.
//!
//! Run with: `cargo run -p dstate-ractor --example node_resource`
...
/// Full internal state — may contain fields not shared with peers.
#[derive(Clone, Debug)]
struct NodeResourceState {
    cpu_percent: f64,
    memory_used_mb: u64,
    memory_total_mb: u64,
    /// Internal-only: number of samples collected (not shared with peers).
    sample_count: u64,
}

/// Public view visible to all peers.
...
/// Incremental update to the view.
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize)]
...
/// Description of what changed in a mutation.
#[derive(Clone, Debug)]
enum ResourceChange {
...
// ---------------------------------------------------------------------------
// DeltaDistributedState implementation
// ---------------------------------------------------------------------------
...
impl DeltaDistributedState for NodeResource {
    ...
    fn sync_urgency(
        ...
        // Push immediately if memory usage changed by more than 10%
    ...
    fn serialize_state(state: &NodeResourceState) -> Vec<u8> {
        // For persistence, serialize only the public fields
...
// ---------------------------------------------------------------------------
// Main — demonstrates spawning actors and using timers with RactorRuntime
// ---------------------------------------------------------------------------
...
async fn main() {
    ...
    println!("=== dstate Node Resource Monitor (ractor) ===\n");
    println!(
    ...
    // Shared state for the monitor actor
    ...
    // Demonstrate send_interval: periodic sampling every 200ms
    ...
    // Send a one-shot high-memory sample via send_after
    ...
    // Demonstrate delta serialization round-trip
    println!("\n--- Delta serialization round-trip ---");
    let delta = NodeResource::project_delta(&ResourceChange::CpuUpdated {
        old: 45.2,
        new: 92.5,
    });
