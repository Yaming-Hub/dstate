...
use dstate::{
    ActorRef as DstateActorRef, ActorRuntime as DstateActorRuntime, ActorSendError, GroupError,
    TimerHandle,
};
...
/// A dstate `ActorRef` backed by a kameo `ActorRef<HandlerActor<M>>`.
///
...
impl<M: Send + 'static> DstateActorRef<M> for KameoActorRef<M> {
...
/// A dstate `ActorRuntime` implementation backed by kameo.
///
...
impl DstateActorRuntime for KameoRuntime {
